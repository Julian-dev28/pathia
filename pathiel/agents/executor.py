...
from pathiel.agents.risk_gates import GateContext, eval_all_gates
...
def maybe_execute(analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``analysis`` is the shape documented as `pathiel.models.types.
    ...
    tool call in `scripts/pathiel-mcp-server.py`) where the static type is
    ...
    if gate_output["blocked"]:
        ...
        if _relax_admit:
            ...
            if _is_thin_short_reject and float(analysis.get("confidence", 0) or 0) >= 0.70:
                try:
                    from pathiel.agents import shadow_ledger as _sl
    ...
    try:
        ...
        try:
            ...
            _funding_hr = None
        try:
            from pathiel.agents.market_regime import detect_regime as _detect_regime
...
def _daily_ma_direction(coin: str, period: int) -> int:
    ...
    try:
        ...
        from pathiel.indicators.math import candle_val, ema
