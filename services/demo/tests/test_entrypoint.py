...
What they defend:
  ...
  - The credential guard. `PATHIEL_PUBLIC_DASHBOARD=1` opens every account
  ...
  - Statement order. Three separate module-scope side effects in the pathiel
...
def _lineno_of_pathiel_import(tree: ast.Module) -> int:
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and (node.module or "").startswith("pathiel"):
            return node.lineno
    pytest.fail("api/index.py never imports pathiel — it cannot serve the dashboard")
...
class TestCredentialGuard:
    def test_names_every_credential_that_would_matter(self, source: str):
        for key in ("HYPERLIQUID_PRIVATE_KEY", "PRIVATE_KEY_HEX",
                    "HYPERLIQUID_ACCOUNT_ADDRESS", "PATHIEL_OPERATOR_TOKEN"):
    ...
    def test_treats_a_local_env_file_as_a_credential(self, source: str):
        """`pathiel/server.py` loads `.env.local` into the environment on import,
    ...
    def test_guard_runs_before_the_dashboard_is_imported(self, tree: ast.Module):
        ...
        assert raise_line < _lineno_of_pathiel_import(tree)


class TestEnvironmentOrder:
    @pytest.mark.parametrize("key", [
        "HOME",
        "PATHIEL_PUBLIC_DASHBOARD",
        "PATHIEL_DASHBOARD_READONLY",
    ])
    def test_set_before_pathiel_is_imported(self, tree: ast.Module, key: str):
        assert _lineno_of_env_assignment(tree, key) < _lineno_of_pathiel_import(tree), (
            f"{key} is set after pathiel is imported; the module-scope constants "
    ...
    def test_home_is_redirected_somewhere_writable(self, source: str):
        ...
        because `pathiel/client/universe.py` mkdirs `Path.home()/.pathiel`."""
...
class TestReadOnly:
    def test_write_path_is_closed(self, source: str):
        assert 'os.environ["PATHIEL_DASHBOARD_READONLY"] = "1"' in source

    def test_public_read_is_open(self, source: str):
        """Without this the demo renders a sign-in prompt and no data."""
        assert 'os.environ["PATHIEL_PUBLIC_DASHBOARD"] = "1"' in source
...
class TestSiweDomain:
    ...
    def test_the_entrypoint_sets_the_auth_domain(self, source: str):
        assert "PATHIEL_AUTH_DOMAIN" in source
    ...
    def test_the_domain_is_set_before_pathiel_is_imported(self, tree: ast.Module):
        ...
        setdefault_lines = [
            ...
            and any(isinstance(a, ast.Constant) and a.value == "PATHIEL_AUTH_DOMAIN"
                    for a in n.args)
        ]
        assert setdefault_lines, "PATHIEL_AUTH_DOMAIN is never set"
        assert min(setdefault_lines) < _lineno_of_pathiel_import(tree)
...
class TestWritableState:
    ...
    universe cache, PATHIEL_STATE_DIR for the auth database. Both have now broken
    ...
    def test_a_writable_state_dir_is_set(self, source: str):
        """services/auth builds `<PATHIEL_STATE_DIR>/auth.db`, defaulting to "."."""
        assert "PATHIEL_STATE_DIR" in source
        assert "gettempdir()" in source

    def test_it_is_set_before_pathiel_is_imported(self, tree: ast.Module):
        ...
        lines = [n.lineno for n in ast.walk(tree)
                 if isinstance(n, ast.Call) and isinstance(n.func, ast.Attribute)
                 and n.func.attr == "setdefault"
                 and any(isinstance(a, ast.Constant) and a.value == "PATHIEL_STATE_DIR"
                         for a in n.args)]
        assert lines, "PATHIEL_STATE_DIR is never set"
        assert min(lines) < _lineno_of_pathiel_import(tree)
