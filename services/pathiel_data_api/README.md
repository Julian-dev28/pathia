...
├── sdk/
│   ├── pathiel_client.py     # typed Python client (httpx)
...
- **The two services do not import each other.** `services/auth` reaches this
  ...
  `test_dockerfile_does_not_bundle_pathiel_data_api` enforces that. The contract
...
request path. The SDK reads this body back into `PathielAPIError` (§11).
...
Config (env, `PATHIEL_` prefix):

| Setting | Env | Default | Meaning |
|---|---|---|---|
| `rate_limit_per_min` | `PATHIEL_RATE_LIMIT_PER_MIN` | 500 | sustained refill rate |
| `rate_limit_burst` | `PATHIEL_RATE_LIMIT_BURST` | 60 | bucket capacity |
...
- `request_id` is a `ContextVar` set by middleware (`new_request_id()`), echoed
  ...
  request.
- `setup_logging(level, as_json)` installs the formatter; `PATHIEL_LOG_JSON=false`
...
cleanly to both; switch `PATHIEL_DATABASE_URL` to a `postgresql://...` DSN.
...
1. **Hand-written client** — `sdk/pathiel_client.py`. `PathielDataClient(base_url,
   api_key)` with `health()`, `ohlc()`, `net_flow()`, `momentum()`. Bearer auth,
   unwraps the `data` envelope, raises `PathielAPIError` (with `title` / `detail`
...
- **Gate tests** — `tests/test_api.py`, deterministic, network-free, fast. They
  ...
  `Retry-After`.
- **Hermeticity** — `tests/conftest.py` points `PATHIEL_DATABASE_URL` at a
...
```bash
cd services/pathiel_data_api
python -m pytest tests -v
# from repo root, just this service:
python -m pytest services/pathiel_data_api/tests -v
```
...
COPY app ./app
EXPOSE 8000
# PATHIEL_* env supplies DB URL, rate limits, and (optionally) a licensed feed URL.
CMD ["uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8000"]
```

```bash
docker build -t pathiel-data-api:local .
docker run --rm -p 8000:8000 \
  -e PATHIEL_DATABASE_URL=sqlite:////data/pathiel_data.db \
  -e PATHIEL_RATE_LIMIT_PER_MIN=500 \
  -v pathiel_data:/data \
  pathiel-data-api:local
```

For production, point `PATHIEL_DATABASE_URL` at Postgres and drop the volume.
...
apiVersion: apps/v1
kind: Deployment
metadata: {name: pathiel-data-api, namespace: pathiel}
spec:
  replicas: 2
  selector: {matchLabels: {app: pathiel-data-api}}
  template:
    metadata: {labels: {app: pathiel-data-api}}
    spec:
      containers:
        - name: web
          image: pathiel-data-api:local
          ports: [{name: http, containerPort: 8000}]
          envFrom:
            - configMapRef: {name: pathiel-data-config}   # rate limits, feed URLs
            - secretRef: {name: pathiel-data-secrets}      # DATABASE_URL, licensed keys
...
jobs:
  test:
    ...
    steps:
      ...
      - run: python -m pytest services/pathiel_data_api/tests -v
...
cd services/pathiel_data_api
export PATHIEL_DATABASE_URL="sqlite:///./pathiel_data.db"
